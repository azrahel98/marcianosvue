  ...
  <form @submit.prevent="submitOrder" class="space-y-4">
    ...
    <div class="bg-blue-50/50 p-3 rounded-xl border border-blue-100 shadow-sm mb-4 relative z-20">
      ...
      <div class="relative group">
        <select
          v-model="pedido.clientId"
          class="h-10 w-full rounded-lg bg-white border border-blue-200 pl-3 pr-8 text-xs font-medium text-gray-700 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all appearance-none cursor-pointer hover:border-blue-300"
          required
        >
          <option :value="0" disabled>Seleccione un cliente...</option>
          <option v-for="cliente in adminStore.clientes" :key="cliente.id_usuario" :value="cliente.id_usuario">
            {{ cliente.nombre }} (Puntos: {{ cliente.puntos || 0 }})
          </option>
        </select>
        <div class="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none text-blue-400 group-hover:text-blue-500 transition-colors">
    ...
    <!-- Lista de artículos -->
    <div class="space-y-2 relative min-h-[120px] z-0">
      <div v-if="pedido.items.length === 0" class="flex flex-col items-center justify-center py-8 text-gray-300 border-2 border-dashed border-gray-100 rounded-xl h-full">
        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="mb-2 opacity-50"><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4Z" /><path d="M3 6h18" /><path d="M16 10a4 4 0 0 1-8 0" /></svg>
        <span class="text-xs font-medium">El pedido está vacío</span>
      </div>
    ...
    <div class="pt-4 mt-2 border-t border-gray-50">
      ...
      <div class="flex gap-3">
        ...
        <button
          type="submit"
          :disabled="isSubmittingOrder || pedido.items.length === 0 || pedido.clientId === 0"
...
const submitOrder = async () => {
  if (pedido.value.items.length === 0 || pedido.value.clientId === 0) return

  try {
    isSubmittingOrder.value = true

    const productos = pedido.value.items.map((item) => ({
      idSabor: item.saborid,
      cantidad: item.cantidad
    }))

    // El estado default 'pendiente' está bien
    await adminStore.crearPedidoParaCliente(pedido.value.clientId, productos, 'pendiente')

    emit('refresh')
    emit('close')
  } catch (error) {
    alert((error as Error).message || 'Error al crear pedido (admin)')
    console.error(error)
  } finally {
