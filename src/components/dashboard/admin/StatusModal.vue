  ...
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" @click.self="emit('close')">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden transform transition-all">
      ...
      <div class="p-6">
        ...
        <div class="space-y-2">
          <label for="status" class="block text-xs font-medium text-gray-700">Nuevo Estado</label>
          <select id="status" v-model="selectedStatus" class="block w-full rounded-lg border-gray-300 bg-gray-50 p-2.5 text-sm focus:border-pink-500 focus:ring-pink-500">
